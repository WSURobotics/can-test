...
int XiaomiCyberGearDriver::reset_twai(uint8_t rx_pin, uint8_t tx_pin, bool serial_debug/*=false*/){
    _use_serial_debug = serial_debug;
    ...
    if (twai_driver_uninstall() == ESP_OK) {
        if (_use_serial_debug) Serial.println("Driver uninstalled");
    } else {
        if (_use_serial_debug) Serial.println("Failed to uninstall driver");
        return -1;
    }
...
void XiaomiCyberGearDriver::_send_can_package(uint8_t can_id, uint8_t cmd_id, uint16_t option, uint8_t len, uint8_t* data){
    ...
    } else if (twai_transmit(&message, pdMS_TO_TICKS(1000)) == ESP_ERR_INVALID_STATE) {
        // jank af
        reset_twai(4, 5, true);
        init_twai(4, 5, true);
    } else {
